...
//		d.reset();
			...
			public void messageReceived(Bead msg) {
				...
				if(d.clock.getCount() % (8 - beatInterval) == 0) {
					...
					if(!birdMode) {
						count++;
						if(count % 20 == 0) {
							d.synch.broadcast("guit " + d.myIndex() + " ");
						}
					}
		...
		//listen to broadcast message from others
	...
	private void updateState() {
		...
		//either bird mode or go to tones with chords
		
		
		if(d.rng.nextFloat() < 0.001f) {
			birdMode = !birdMode;
